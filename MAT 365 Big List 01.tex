...
\section{Algebraic topology}
    ...
    \subsection{Algebra primer}
        ...
        %
        \item \textbf{Generators and direct sums:}

            \defn{Let $G$ be abelian and $\{G_\alpha\}_J$ be a family of subgroups of $G$. The $G_\alpha$ generate $G$ if every element $x\in G$ can be written as a finite sum $x = x_{\alpha_1} + \cdots + x_{\alpha_n}$.

            \textit{We may write $x$ as a \textsc{formal sum}: \[x = \sum_{\alpha\in J} x_\alpha\]}
        %
        \item free groups, direct sums, free/internal/external products, etc.
        %
        \end{enumerate}
